...
execute unless predicate {"type":"minecraft:int_value_check","value":{"type":"minecraft:storage","storage":"eden:temp","path":"clock.exp_level"},"test":{"min":{"type":"minecraft:storage","storage":"eden:settings","path":"warping_wonders.clock.exp_cost"}}} run return run title @s actionbar {"bold":false,"color":"red","italic":false,"translate":"message.warping_wonders.general.insufficient_exp"}
...
execute as @s[gamemode=!creative] if predicate {"type":"minecraft:random_chance","chance":{"type":"minecraft:storage","storage":"eden:settings","path":"warping_wonders.clock.breaking_chance"}} run function wawo:remove_used_item {item:"minecraft:clock"}
