...
$execute if data storage eden:database waypoints.hubs.$(id){access:"private"} unless data storage eden:database waypoints.hubs.$(id).profile{id:$(uuid)} run return run tellraw @s [{"bold":false,"color":"dark_purple","italic":false,"text":"▊ "},{"bold":false,"color":"white","italic":false,"translate":"message.warping_wonders.waypoint_hub.no_access"}]
...
$execute at @s unless predicate wawo:entity/is_riding in $(dimension) positioned $(x) $(y) $(z) run tp @s ~-1 ~.5 ~-1
$execute at @s if predicate wawo:entity/is_riding in $(dimension) positioned $(x) $(y) $(z) run tp @n[type=#wawo:is_mount] ~-1 ~.5 ~-1
