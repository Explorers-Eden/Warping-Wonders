...
function wawo:waypoint_hub/interaction/left_click/check_uuid with storage eden:temp waypoint.head

execute as @e[type=minecraft:interaction,tag=wawo.waypoint.hub.interaction,distance=..16] if data entity @s attack run data remove entity @s attack